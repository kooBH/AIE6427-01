...
###################################################################
ENHANCE_MODEL_PATH = "./Unet_V2.pth"
###################################################################
...
class GrandChallengeDataset_Test(Dataset):
    ...
    def __init__(self, data_test):
        ...
        self.test_spec = torchaudio.functional.spectrogram(waveform=self.test_wav,
                                                           pad=0, window=self.window, n_fft=self.win_len,
                                                           hop_length=self.hop_len, win_length=self.win_len,
                                                           power=None, normalized=False,
                                                           )
        self.test_spec = torch.view_as_real(self.test_spec[0])  # 1 F T 2 --> F T 2
...
def enhance(data_ch1_list,dir_out,path_model):
    debug = []

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    batch_size = 1
    test_data_output_path = dir_out
    ...
    window = torch.hann_window(window_length=int(1024), periodic=True, dtype=None, layout=torch.strided, device=None,
                               requires_grad=False).to(device)

    with torch.no_grad():
        for i in tqdm(range(len(data_ch1_list))):
            test_dataset = GrandChallengeDataset_Test(data_ch1_list[i])
            test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False,
                                                      num_workers=0)
            flag = 0
            for j, (data_name, out_data_real, out_data_imag, tgt_wav_len, data_full_name) in enumerate(test_loader):
                if j == 0:
                    audio_real = out_data_real.to(device)
                    audio_imagine = out_data_imag.to(device)
                    ...
                    debug = debug + [audio_real, audio_imagine, enhance_r, enhance_i]
                else:
                    audio_real = out_data_real.to(device)
                    audio_imagine = out_data_imag.to(device)
                    ...
                    if flag == 0:
                        debug = debug + [enhance_r, enhance_i]
                    flag = 1
                    

            audio_max_len = int(enhance_spec.shape[2] * 256 - 1)
            audio_me_pe = complex2audio(enhance_spec, window, audio_max_len)
            ...
            data_item = data_full_name[0]

            audio_me_pe = audio_me_pe.to('cpu')
            output_wav = audio_me_pe[:, :int(tgt_wav_len[0])]

            if not os.path.exists(audiosave_path + "/enhance_wav/"):
                os.makedirs(audiosave_path + "/enhance_wav/")

            output_wav = output_wav.numpy()
            #np.save(audiosave_path + "/enhance_wav/" + data_name + ".npy", output_wav)
            sf.write(audiosave_path + "/enhance_wav/" + data_name + ".wav", output_wav.T,16000)
            batch_dict = {"output_path": audiosave_path + "/enhance_wav/" + data_name + ".npy", "input_path": data_item}

            save_folder_name = os.path.join(audiosave_path, "pickle", "1_enhanced")
            os.makedirs(save_folder_name, exist_ok=True)

            with open(os.path.join(save_folder_name, data_name + '.pkl'), 'wb') as f:
                pickle.dump(batch_dict, f)
    return debug

def trt_enhance(data_ch1_list,dir_out,trt_engine_path):
    debug = []
    ...
    with torch.no_grad():
        for i in tqdm(range(len(data_ch1_list))):
            test_dataset = GrandChallengeDataset_Test(data_ch1_list[i])
            print(f"processing {data_ch1_list[i]}")
            test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False,
                                                      num_workers=0)
            flag = 0
            for j, (data_name, out_data_real, out_data_imag, tgt_wav_len, data_full_name) in enumerate(test_loader):
                if j == 0:
                    audio_real = out_data_real.numpy()
                    audio_imagine = out_data_imag.numpy()
                    enhance_r, enhance_i = model.inference(audio_real, audio_imagine)
                    enhance_r, enhance_i = torch.Tensor(enhance_r), torch.Tensor(enhance_i)
                    enhance_r = enhance_r.unsqueeze(3)
                    enhance_i = enhance_i.unsqueeze(3)
                    enhance_spec = torch.cat((enhance_r, enhance_i), 3)
                    debug = debug + [audio_real, audio_imagine, enhance_r, enhance_i]
                    ...
                    enhance_r, enhance_i = model.inference(audio_real, audio_imagine).astype(np.float32)
                    enhance_r, enhance_i = torch.Tensor(enhance_r), torch.Tensor(enhance_i)
                    enhance_r = enhance_r.unsqueeze(3)
                    enhance_i = enhance_i.unsqueeze(3)
                    enhance_spec2 = torch.cat((enhance_r, enhance_i), 3)
...
def enhance_diff(ata_ch1_list,dir_out,trt_engine_path, path_model):
    return

if __name__ == "__main__":
    root = "/home/nas4/user/kjh4/aig2022/"
    list_target = glob.glob(os.path.join(root, "shorts","*.wav"))
    t_debug = enhance(list_target, os.path.join(root, "torch_out"), "Unet_V2.pth")
    trt_engine_path = os.path.join(root,"output_2.engine")
    trt_debug = trt_enhance(list_target, os.path.join(root, "trt_out"), trt_engine_path)
    print(len(trt_debug))
    for i, (torch_debug_data, trt_debug_data) in enumerate(zip(t_debug,trt_debug)):
        if torch_debug_data.shape == trt_debug_data.shape:
            print(f"debug data {i} is same,  torch:{torch_debug_data.shape}, trt:{trt_debug_data.shape}")
        else:
            print(f'idx:{i}, torch:{torch_debug_data.shape}, trt:{trt_debug_data.shape}')
            # print(f"vs TRT [{i}] : ", np.allclose(torch_debug_data.cpu().numpy(), trt_debug_data.cpu().numpy(), rtol=1e-4, atol=0))
